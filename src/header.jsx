...
const Header = ({ isSidebarOpen, toggleSidebar }) => {
  ...
  return (
    <header
      className={`top-0 h-16 text-white flex justify-between p-2 transition-all duration-300 ease-in-out ${
    ...
    >
      <div className="flex items-center space-x-2 p-4 rounded-lg">
        {actionButtons
          ...
          .map((btn, idx) => (
            <div key={idx} className={iconButtonClass} onClick={btn.onClick}>
              {btn.icon}
            </div>
          ))}

        <div className="flex items-center space-x-2 p-2 rounded-lg hover:bg-stone-800">
      ...
      <div className="flex items-center p-4 space-x-4">
        <button
          className="px-4 py-2 text-white rounded-full border border-stone-600 flex items-center space-x-2 hover:bg-stone-800"
          onClick={() => alert("Share")}
        >
